...
class CinematographerService(ICinematographer):
    ...
    def _load_reference_poses(self) -> List[Tuple[bytes, str]]:
        """Load reference pose images with fallback logic."""
    ...
    def _generate_keyframe_a(self, plan: VideoPlan, reference_pose_index: int) -> Tuple[str, Any]:
        """Generate Keyframe A (Master Shot)."""
    ...
    def _generate_sequential_keyframes(self, plan: VideoPlan, variety_instruction: str, initial_response: Any) -> Dict[str, str]:
        """Generate subsequent keyframes (B, C, D...) sequentially."""
        assets = {}

        # Initialize history and previous response for the loop
        current_history = None
        previous_response_parts = initial_response.parts
    ...
    def generate_assets(self, plan: VideoPlan, output_dir: str = None, scene_variety: int = None, reference_pose_index: int = 0) -> Dict[str, str]:
        ...
        try:
            ...
            sequential_assets = self._generate_sequential_keyframes(plan, variety_instruction, response_a)
