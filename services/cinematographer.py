...
class CinematographerService(ICinematographer):
    ...
    def _generate_keyframe_a(self, plan: VideoPlan, reference_pose_index: int) -> tuple[str, Any]:
        """Generate the first keyframe (Keyframe A) using the master prompt and optional reference pose.

        Returns:
            Tuple of (file_path, api_response_parts)
        """
    ...
    def _generate_subsequent_keyframes(self, plan: VideoPlan, variety_instruction: str, previous_response_parts: Any) -> Dict[str, str]:
        """Generate subsequent keyframes (B, C, D...) based on the plan and history."""
        assets = {}
        current_history: List[types.Content] = []
    ...
    def generate_assets(self, plan: VideoPlan, output_dir: str = None, scene_variety: int = None, reference_pose_index: int = 0) -> Dict[str, str]:
        ...
        try:
            ...
            subsequent_assets = self._generate_subsequent_keyframes(plan, variety_instruction, parts_a)
            assets.update(subsequent_assets)
